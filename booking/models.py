...
class Booking(PostgresPartitionedModel):
    ...
    def clean(self):
        ...
        room_conflicts = Booking.objects.filter(
            room=self.room,
            date=self.date,
        ...
        if room_conflicts.exists():
            raise ValidationError("This booking conflicts with an existing booking in this room.")

        # 3) Конфликт по пользователю: чтобы у одного юзера не было пересекающихся броней в разных комнатах
        user_conflicts = Booking.objects.filter(
            user=self.user,
            date=self.date,
        ).exclude(pk=self.pk).filter(
            start_time__lt=self.end_time,
        ...
        if user_conflicts.exists():
            raise ValidationError("You already have another booking at this time in a different room.")
