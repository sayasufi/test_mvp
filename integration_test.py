...
try:
    ...
    cur = conn.cursor()
    cur.execute(
        "UPDATE auth_user SET is_staff = TRUE WHERE username = %s",
        (ADMIN_CRED["username"],),
    )
    conn.commit()
    cur.close()
...
def test_rooms():
    ...
    r = session.post(
        base, json={"name": "R1", "capacity": 5, "floor": 1}, headers=user_headers
    )
    log(
        "Создание комнаты обычным пользователем",
        r.status_code == 403,
        f"status={r.status_code}, body={r.text}",
    )
    # создание комнаты администратором
    r2 = session.post(
        base, json={"name": "R1", "capacity": 5, "floor": 1}, headers=admin_headers
    )
    ...
    # обновление комнаты
    r4 = session.patch(
        f"{base}{room_id}/", json={"capacity": 10}, headers=admin_headers
    )
...
def test_free_rooms():
    ...
    r = session.post(
        f"{BASE_URL}/rooms/",
        json={"name": unique, "capacity": 3, "floor": 2},
    ...
    info = (
        ""
        if ok
        else f"status={r3.status_code}, returned_ids={[rm['id'] for rm in results_free]}, full_response={r3.json()}"
    )
...
def test_bookings():
    ...
    r = session.post(
        f"{BASE_URL}/rooms/",
        json={"name": unique, "capacity": 4, "floor": 3},
        headers=admin_headers,
    )
    if r.status_code == 201:
        room3 = r.json().get("id")
    ...
    log(
        "Просмотр бронирований пользователем",
        ok,
        f"status={r3.status_code}, body={r3.json()}",
    )
    ...
    log(
        "Просмотр бронирований администратором",
        ok,
        f"status={r4.status_code}, body={r4.json()}",
    )
    # обновление брони
    r5 = session.patch(
        f"{BASE_URL}/bookings/{bid}/",
        json={"end_time": "11:15:00"},
        headers=user_headers,
    )
...
def load_test_free(concurrency=10, per_thread=10):
    def worker(_):
        ...
        for __ in range(per_thread):
            resp = session.get(
                f"{BASE_URL}/rooms/free/", params=params, headers=user_headers
            )
